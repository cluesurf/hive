...
In Euclidean space, panning is simple: drag the mouse by (dx, dy) pixels,
translate all points by (-dx, -dy). But hyperbolic space is different.
There's no global translation. Instead, we apply hyperbolic isometries
(distance-preserving transformations) that move the "viewpoint" through
the space.
...
- Distances are non-linear (things shrink exponentially toward the boundary)
...
The tricky part: a 100-pixel drag near the center of the disk should move
you a different hyperbolic distance than a 100-pixel drag near the edge.
...
For more accuracy, compute the actual hyperbolic distance between A and B.
...
This composition means: first apply current view, then apply the new drag.
...
1. **Normalize after transform**: Points may drift off the hyperboloid due
   to floating point errors. Re-normalize after each transform.
...
3. **WebGL batching**: For many tiles, batch the matrix multiply in a shader
